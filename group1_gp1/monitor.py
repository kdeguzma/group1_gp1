...
class Monitor(Node):
    ...
    def __init__(self, node_name:str) -> None:
        ...
        self._qos_pub = QoSProfile(depth=1, reliability = ReliabilityPolicy.RELIABLE, durability = DurabilityPolicy.VOLATILE)
        self._qos_sub = QoSProfile(depth=3, reliability = ReliabilityPolicy.BEST_EFFORT, durability = DurabilityPolicy.VOLATILE)
        ...
        self._subscriber = self.create_subscription(String,"/fleet/status", self._subscriber_callback, self._qos_sub, callback_group = self._sub_callback_group)

        
        self._robot_last_task = {"robot_1":None,"robot_2":None,"robot_3":None}
        self._robot_completed = {"robot_1":0,"robot_2":0,"robot_3":0}
        self._robot_status_ts = {"robot_1":self.get_clock().now(),"robot_2":self.get_clock().now(),"robot_3":self.get_clock().now()}
        
        self._timer = self.create_timer(2.0,self._timer_callback, callback_group = self._pub_callback_group)
