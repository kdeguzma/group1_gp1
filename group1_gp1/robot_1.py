...
# Group 1
# robot_1.py - Subscribes to /fleet/tasks, publishes to /fleet/status, and filters messages for id.
...
class RobotOne(Node):
    """Class for robot_1's node which subscribes to /fleet/tasks and publishes to /fleet/status."""

    def __init__(self) -> None:
        """Initializer function for the robot_1 sub/pub node."""
        # Initialize the node with the name "robot_1".
    ...
    def _subscriber_callback(self, message: String) -> None:
        ...
        try:
            ...
            # Filter out tasks that are not robot_1's ID.
            if (
                robot_id != self._robot_id
            ):  # or self._task is not None: # Commented out: Prevent accepting new tasks while busy.
                return

            # Otherwise, accept and set the task as robot_1's attribute.
            self._task = task
